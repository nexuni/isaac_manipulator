...
2. Start the main launch file
    ```bash
    ros2 launch isaac_manipulator_pick_and_place ur_hande_pick_and_place.launch.py ur_type:=ur10e robot_ip:=<YOUR ROBOT IP> gripper_type:=robotiq_hande camera_type:=realsense setup:=<YOUR SETUP NAME> use_pose_from_rviz:=True num_cameras:=1 kinematics_parameters_file:=my_robot_calibration.yaml yolov8_object_class_id:=<YOUT TARGET CLASS ID> object_attachment_type:=cuboid object_attachment_scale:=<YOUR OBJECT SIZE>
    ```

    Here is an example command:
    ```bash
    ros2 launch isaac_manipulator_pick_and_place ur_hande_pick_and_place.launch.py ur_type:=ur10e robot_ip:=10.0.0.155 gripper_type:=robotiq_hande camera_type:=realsense setup:=nexuni_demo use_pose_from_rviz:=True num_cameras:=1 kinematics_parameters_file:=my_robot_calibration.yaml yolov8_object_class_id:=5 object_attachment_type:=cuboid object_attachment_scale:=[0.11,0.033,0.09]
    ```
    ...
    * The prarmeter `object_attachment_scale` is used as attaching an fake cubid to the tool, which aims to consider the grasped object as planning. The unit is meter and it should be like `[0.11,0.033,0.09]`, the three numbers correspond to the size along x-, y-, and z-axis. However, there is no guarantee that z-axis always points upeard. The only way to check this is observing the tf frame generated by foundationpose on Rviz. Red arrow is x-axis, green one is y-axis, and blue one is z-axis.
...
4. Setup your goal pose by dragging and rotating the orange pose on Rviz. Then, the script will go to that pose after grasping the object.

5. Send a command to trigger the rest of the pipeline, including foundation pose, grasp pose estimation, planning, gripper controlling.
    ```bash
...
6. If you want to trigger again, you need to run tthis command first, and then go back to step3.
    ```bash
...
The scripts and commands above is to make the arm grasp the object and go to the target pose (actually the target joint state). There are different ways to control the arm with some scripts or commands modification.

#### Setup Target Pose with Cmd

If you want the arm goes to the same goal pose (position and orientation of the fram `tool0`), you could send the target pose with the `PickAndPlace` action. There are 2 commands need to be edited (**Keep those commmands which are not mentioned the same**):

1. Start the main launch file with 
    ```bash
    ros2 launch isaac_manipulator_pick_and_place ur_hande_pick_and_place.launch.py ur_type:=ur10e robot_ip:=<YOUR ROBOT IP> gripper_type:=robotiq_hande camera_type:=realsense setup:=<YOUR SETUP NAME> use_pose_from_rviz:=False num_cameras:=1 kinematics_parameters_file:=my_robot_calibration.yaml yolov8_object_class_id:=<YOUT TARGET CLASS ID> object_attachment_type:=cuboid object_attachment_scale:=<YOUR OBJECT SIZE>

    Change `use_pose_from_rviz` to `False`

2. Trigger the rest of the pipeline with
    ...
    You should find your place_pose bu yourself.

    * You could get the exact number to your traget pose with `tf2_echo`. The pose we used in the command is the frame transformation from `base_link` to `grasp_frame`. Than means that you could move the arm to where you want and then record the pose by running
    ```bash
    ros2 run tf2_ros tf2_echo base_link grasp_frame
    ```
    Noted that the main launch needs to be executed as getting the tf.

#### Pick and Place Repeatedly

The previous setups could only run one time. Then, you need to clearObject and send another action by yourself. There is another setup makes is possible to run a single node which will trigger the actions automatically. 

1. Comment `isaac_manipulator_pick_and_place/scripts/pick_and_place_orchestrator.py` Line-136 and uncomment Line-137. Then, the script will pick the target from A to B, B to A, .... The pose A and B are hardcoded in `isaac_manipulator_pick_and_place/scripts/pick_and_place_orchestrator.py` Line-678 - 704. You could edit to your own target pose, which got with the same way mentioned above.

2. After editing the code, remember re-compiling the package with `colcon build`. If you compiled the package with `--symlink-install`, you could skiop this step.

3. Instead of running Step3-6 in the [section](#run-the-whole-pipeline). You only need to run another ros node with
    ```bash
    ros2 run isaac_manipulator_pick_and_place pick_and_place_executor.py --ros-args -p target_class_id:=<YOUT TARGET CLASS ID>
    ```

    Then, this node will handle the actions and service in Step3-6. Automatically check which object belongs to your target class and trigger the pickAndPlace process.